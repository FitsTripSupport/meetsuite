import React, {useEffect, useState} from 'react';
import {useCountries, usePackagesByCountry} from '../hooks/useMeetSuiteLocations';

const CMBServices = () => {
    const {countries, loading: loadingCountries, error: countriesError} = useCountries();
    const [selectedCountry, setSelectedCountry] = useState('');

    // once countries are loaded, default to the first one
    useEffect(() => {
        if (!selectedCountry && countries.length > 0) {
            setSelectedCountry(countries[0]);
        }
    }, [countries, selectedCountry]);

    const {
        packages,
        loading: loadingPackages,
        error: packagesError,
    } = usePackagesByCountry(selectedCountry);

    const handleCountryClick = (country) => {
        setSelectedCountry(country);
    };

    return (
        <section
            className="bg-[#f8f5f0] py-12 bg-cover bg-center bg-no-repeat"
            style={{backgroundImage: `url('/asset/airport.jpg')`}}
            id="services"
            name="services"
        >
            <div className="max-w-6xl mx-auto px-4">
                <h2 className="text-3xl font-extrabold text-left mb-6 text-[#000]">
                    Meetsuite Experience
                </h2>

                {/* Errors (if any) */}
                {countriesError && (
                    <p className="text-red-600 mb-4 text-sm">
                        Failed to load countries: {countriesError}
                    </p>
                )}
                {packagesError && (
                    <p className="text-red-600 mb-4 text-sm">
                        Failed to load services: {packagesError}
                    </p>
                )}

                {/* Country Buttons */}
                <div className="flex flex-wrap gap-4 mb-8">
                    {loadingCountries && countries.length === 0 && (
                        <span className="text-sm text-gray-700">Loading countries...</span>
                    )}

                    {countries.map((country) => (
                        <button
                            key={country}
                            onClick={() => handleCountryClick(country)}
                            className={`px-4 py-2 rounded-md w-32 border font-semibold cursor-pointer transition ${
                                selectedCountry === country
                                    ? 'bg-[#000] text-[#BE965B]'
                                    : 'bg-white text-[#BE965B] border-[#000]'
                            }`}
                        >
                            {/* <span className="mr-2">{countryFlags[country]}</span> */}
                            {country}
                        </button>
                    ))}
                </div>

                {/* Service Cards */}
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {loadingPackages && (
                        <p className="col-span-full text-center text-gray-700">
                            Loading services...
                        </p>
                    )}

                    {!loadingPackages && packages.length === 0 && selectedCountry && (
                        <p className="col-span-full text-center text-gray-500">
                            No services available for {selectedCountry} yet.
                        </p>
                    )}

                    {packages.map((pkg, index) => {
                        // Map backend DTO shape to your old JSON structure
                        const serviceName = pkg.packageName;
                        const inclusions = (pkg.inclusions || []).map((incl) => incl.detail);
                        // const priceText = pkg.price ? `USD ${pkg.price}` : '';

                        return (
                            <div
                                key={pkg.id || index}
                                className="bg-gradient-to-br from-[#f6e27a] to-[#c49b3d] hover:from-[#c49b3d] hover:to-[#f6e27a] rounded-2xl shadow-lg p-6 transition-all duration-300 hover:shadow-xl"
                            >
                                <div className="mb-4">
                                    {/* <p className="text-sm text-white font-medium">{pkg.provider}</p> */}
                                    <h3 className="text-xl font-bold text-[#000]">{serviceName}</h3>
                                </div>
                                <ul className="space-y-2 text-sm text-[#000] mb-4">
                                    {inclusions.map((item, i) => (
                                        <li key={i} className="flex items-start">
                                            {/* Optional: <FaCheckCircle className="text-white mt-1 mr-2 w-5 h-5" /> */}
                                            <span>&bull; {item}</span>
                                        </li>
                                    ))}
                                </ul>
                                <div className="text-right font-bold text-[#002366] text-lg bottom-4">
                                    {/* {priceText} */}
                                </div>
                            </div>
                        );
                    })}
                </div>
            </div>
        </section>
    );
};

export default CMBServices;
